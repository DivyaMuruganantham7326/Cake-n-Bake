...
function updateTotalCount() {
    ...
    if (shoppingcart.length == 0) {
        ...
        for (i = 0; i < shoppingcart.length; i++) {
            totalunit += parseInt(shoppingcart[i].quantity);
        }
...
function searchkey() {
    ...
    if (check==true)
    {
        ...
        for (j = 0; j < searchresultlist.length; j++) {
            ...
            productrow += "<td class=\"edit\">" + "<button type=\"button\" id=\"" + indexlist[j] + "\" class=\"editstyle\" data-toggle=\"modal\" data-target='#editProducts' onclick = \"prefillproduct(" + indexlist[j] + ")\";>Edit</button></td>";
            productrow += deletebutton(indexlist[j]);
            productrow += addcartbutton(indexlist[j]);
            productrow += "</tr>";
        }
        document.getElementById("pdiv").innerHTML = "<table id=\"ptable\">" + productrow + "</table>";
    }
    ...
    if (check == false) {
        document.getElementById("pdiv").innerHTML = "No results found";
    }
    userrole_display();
}